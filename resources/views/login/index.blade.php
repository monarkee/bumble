...
@section('content')
    {!! BumbleForm::open(['route' => 'bumble.login.post', 'class' => 'login-form']) !!}
        ...
        <div class="control g-row">
            <div class="g-col-3 tar">
                {!! BumbleForm::label($column, $upper_col, ['class' => 'label label--tar']) !!}
            </div>
            <div class="g-col-9">
                @unless ($column === 'password')
                {!! BumbleForm::text($column, null, ['class' => 'input input1', 'placeholder' => 'Your '.$upper_col]) !!}
                @else
                {!! BumbleForm::password($column, ['class' => 'input input1', 'placeholder' => 'Your '.$upper_col]) !!}
                @endunless
            </div>
        </div>
        @endforeach
        ...
        <div class="login-control g-row">
            <div class="g-col-1">
                {!! BumbleForm::button('Login', ['type' => 'submit', 'class' => 'btn form__btn--auto-with']) !!}
            </div>
